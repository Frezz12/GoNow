...
Для регистрации с кодом подтверждения создайте API key в Resend, подтвердите домен отправителя и добавьте только в локальный `.env` (не коммитить):
...
После регистрации backend отправляет шестизначный одноразовый код. Без первых двух переменных регистрация вернёт `503`, чтобы письмо не было потеряно молча.
