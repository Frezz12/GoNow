...
Возвращает текущую температуру и погодный код для координат. Ручка публичная: она используется клиентами как прокси к погодному провайдеру, чтобы VPN/DNS на устройстве не мешали загрузке виджета.
