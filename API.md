...
- Все запросы и ответы, кроме `/health`, используют JSON и заголовок `Content-Type: application/json`. Исключения: загрузка фотографий использует `multipart/form-data`, скачивание фотографии и ресурсы карты возвращают бинарные данные соответствующего типа.
...
## Ресурсы подложки карты

Эти публичные маршруты изолируют мобильные клиенты от домена поставщика карты. Приложение открывает только `/api/v1/map/style`; остальные URL backend подставляет в стиль и TileJSON автоматически. Авторизация не нужна, потому что MapLibre загружает стиль, тайлы, шрифты и спрайты самостоятельно.

### `GET /api/v1/map/style`

Возвращает готовый JSON-стиль Liberty. Перед ответом backend загружает и стиль, и TileJSON векторного источника, встраивает шаблон тайлов в `sources.openmaptiles` и заменяет все внешние URL на GoNow API. Поэтому клиент не может получить «частичную» карту только с Natural Earth без дорог и городов. Если полный векторный источник недоступен, ручка возвращает `200 OK` с подробной растровой подложкой OpenStreetMap.

Перед отправкой стиль адаптируется для GoNow: подписи населённых пунктов используют одно нативное имя без дублирования латиницей, категории `town` и `village` появляются на более дальнем масштабе, а локальные и региональные дороги становятся заметны раньше. Для городов применяется приоритет OpenMapTiles `rank`, несколько вариантов размещения текста вокруг точки и уменьшенный collision padding; подписи стран, областей и менее важных населённых пунктов не блокируют более приоритетный город. Настройка применяется одинаково ко всем странам, но особенно улучшает плотность подписей и дорожную детализацию России.

При системной DNS-ошибке backend повторяет OpenFreeMap-запрос после DNS-over-HTTPS разрешения адресов через Cloudflare `1.1.1.1`. Полученные IP используются только для TLS-соединения с исходным hostname; проверка сертификата не отключается.

Успешно созданный DNS-over-HTTPS-клиент кэшируется и сразу используется для последующих ресурсов карты. Это исключает ожидание неработающего системного DNS перед каждым тайлом при активном VPN.

### `GET /api/v1/map/planet`

Возвращает TileJSON OpenFreeMap с переписанными шаблонами векторных тайлов. Маршрут сохранён для совместимости и диагностики; основной `/map/style` уже встраивает эти данные и не заставляет MapLibre делать отдельный TileJSON-запрос. Успех: `200 OK`; ошибка внешнего поставщика: `503 Service Unavailable`.

### `GET /api/v1/map/resources/{path}`

Проксирует только разрешённые деревья OpenFreeMap: `natural_earth/`, `planet/`, `sprites/` и `fonts/`. Тип ответа совпадает с ресурсом (`application/json`, vector tile, PNG или PBF). Произвольные адреса и пути с `..` отклоняются, поэтому ручка не является открытым HTTP-прокси. Cache-заголовки поставщика сохраняются.

При размещении backend за reverse proxy нужно передавать `X-Forwarded-Proto` и `X-Forwarded-Host`: по ним сервер формирует абсолютные URL ресурсов. В production публичный OpenFreeMap не имеет SLA; модуль можно переключить на совместимое S3/self-hosted хранилище без изменения iOS-кода.

## Активности и карта

Обе ручки требуют `Authorization: Bearer <accessToken>`. Стабильные категории: `sport`, `walking`, `travel`, `music`, `games`, `help`, `education`, `animals`, `other`.

### `POST /api/v1/activities`

Создаёт опубликованную активность в указанной точке. Текущий клиент передаёт геопозицию телефона и после успешного ответа сразу добавляет новую метку на карту.
...
| --- | --- | --- |
| `title` | Да | От 2 до 120 символов после trim |
| `category` | Да | Один из стабильных идентификаторов выше |
| `latitude` | Да | Число `-90...90` |
| `longitude` | Да | Число `-180...180` |
| `startsAt` | Нет | ISO 8601; при отсутствии используется текущее серверное время |
| `participantLimit` | Нет | Целое число больше нуля или `null` |
...
{
  "data": {
    "id": "0de5d370-7a49-4dc1-b865-7ca7ac68cb58",
    "title": "Вечерняя прогулка",
    "category": "walking",
...
Создатель считается первым участником, поэтому `participantCount` равен `1`, а `isJoined` — `true`. Ошибки: `401 UNAUTHORIZED`; `422 VALIDATION_ERROR` с полями `title`, `category`, `location` или `participantLimit`; `500 INTERNAL_ERROR` при ошибке PostgreSQL.

### `GET /api/v1/activities/map`

Возвращает опубликованные активности внутри фактических границ видимой карты. Запрос выполняется после остановки камеры, а не на каждом кадре движения.

| Параметр query | Тип | Обязателен | Описание |
| --- | --- | --- | --- |
| `south` | number | Да | Южная широта `-90...90` |
| `west` | number | Да | Западная долгота `-180...180` |
| `north` | number | Да | Северная широта `-90...90`, не меньше `south` |
| `east` | number | Да | Восточная долгота `-180...180`; `west > east` означает viewport через antimeridian |
| `zoom` | number | Да | Текущий масштаб карты; сохраняется в контракте для будущей агрегации |
| `categories` | string | Нет | Категории через запятую, например `sport,walking` |
| `startsFrom` | ISO 8601 | Нет | Не возвращать активности раньше этой даты |
| `startsTo` | ISO 8601 | Нет | Не возвращать активности позже этой даты |
| `onlyAvailable` | boolean | Нет | Исключить активности без свободных мест |
| `limit` | integer | Нет | `1...1000`, по умолчанию `500`; сервер жёстко ограничивает максимум |

Пример:

```http
GET /api/v1/activities/map?south=55.4&west=37.2&north=56.0&east=38.0&zoom=11&categories=walking,sport&limit=500
...
```json
{
  "data": {
    ...
    "viewport": { "south": 55.4, "west": 37.2, "north": 56.0, "east": 38.0 }
  },
  "meta": { "count": 1, "truncated": false, "nextCursor": null }
}
```

`truncated: true` означает, что в области больше объектов, чем разрешённый `limit`. Cursor pagination, расстояние, изображения и отдельная модель участников зарезервированы для следующего этапа; поэтому сейчас `nextCursor`, `distanceMeters` и `imageURL` равны `null`.

Первая версия хранит координаты как валидируемые `DOUBLE PRECISION` и использует индексы широты/долготы, чтобы работать с текущим `postgres:17-alpine`. При росте плотности данных схема должна перейти на PostGIS `geography(Point, 4326)` + GiST и server-side aggregation/MVT без изменения публичного DTO.

Ошибки: `400 Bad Request` для query, который нельзя разобрать по типам; `401 UNAUTHORIZED`; `422 VALIDATION_ERROR` для неверных границ или категорий; `500 INTERNAL_ERROR` при ошибке PostgreSQL.
...
| `POST` | `/api/v1/activities` | Создание опубликованной активности по геопозиции |
| `GET` | `/api/v1/activities/map` | Активности в границах видимой карты |
| `GET` | `/api/v1/weather/current` | Текущая погода и локализованное название города |
| `GET` | `/api/v1/map/style` | Стиль карты через GoNow с резервной OSM-подложкой |
| `GET` | `/api/v1/map/planet` | TileJSON с внутренними URL тайлов |
| `GET` | `/api/v1/map/resources/{path}` | Разрешённые тайлы, шрифты и спрайты OpenFreeMap |
