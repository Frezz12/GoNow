...
Возвращает город и текущую погоду для координат: температуру, ощущаемую температуру, влажность, ветер и погодный код. Ручка публичная: она используется клиентами как прокси к погодному провайдеру, чтобы VPN/DNS на устройстве не мешали загрузке виджета.
...
| --- | --- | --- |
| `city` | string \| null | Город, определённый по тем же координатам, что и прогноз. |
